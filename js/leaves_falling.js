...
Leaf1 =new Image();
Leaf1 = allTypeOfLeaves[0] = "images/feather1.png";//path to the image you will use
Leaf2 =new Image();
Leaf2 = allTypeOfLeaves[1] ="images/feather2.png";//you can have up to 3 different kind of leaves
Leaf3 =new Image();
Leaf3 = allTypeOfLeaves[2] ="images/feather3.png";//possible to add more

var speedC = .15;//here you can define speed of leaves
var rotation = .15;//define rotation of leaves
var rotationTrue = 1//whether leaves rotate (1) or not (0)
var numberOfLeaves = 30;//define number of leaves
var size = 120;//general size of leaves, final size is calculated randomly (with this number as general parameter)
...
for (i = 0 ;i < numberOfLeaves;i++){
	...
	if (typeOfLeaf == 3)
	 ...
	 width[i]=Math.round(Math.random()*size+120);//random width and height according to chosen parameter
     height[i]=Math.round(Math.random()*size+120);
	 if (width[i] > height[i]*1.5 || height[i] > width[i]*1.5) 
		width[i] = height[i];
	
	 Ypos[i] = Math.random() * 1080;//starting y position of leaves

	 Xpos[i] = Math.round(Math.random()*(WinWidth));//randomization of x position of leaves
	 opacityLeaf[i] = 0.3;
...
function fallingLeaves(){	
	...
	for (i = 0 ;i < numberOfLeaves;i++)
    {
		...
		Ypos[i]+=strafey;
		Xpos[i]+=strafex; 
		if (Ypos[i] < -240){//setting opacity
			 if(!$.browser.msie){
				opacityLeaf[i]= Math.random()* 0.5;
				$(".leaves"+i).css({opacity:opacityLeaf[i]});			
			}
		}
		if (Ypos[i] > WinHeight - height[i]*2){//leaves slowly disappearing at the end of browser window
			if(!$.browser.msie){
				opacityLeaf[i]-=0.05;	
				$(".leaves"+i).css({opacity:opacityLeaf[i]});
			}
		}
		if (Ypos[i] > WinHeight + height[i]){//when leaves reach certain browser height they are transported back to the begining
			
			Ypos[i]=-Math.random() * WinHeight;
